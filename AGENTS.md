...
- Do not stop or restart production `dashboard`, `homelabd`, `healthd`, or `supervisord` for agent validation. When a requested change requires a supervised component to load new code, restart only the affected component through `homelabctl` or `supervisord`, report the restart impact, and verify the changed workflow against the restarted service.
- Do not add backwards-compatibility fallbacks to support stale supervised component versions unless the operator explicitly asks for multi-version support. If the live service is stale, restart or redeploy the required component instead.
...
- Reuse the dashboard's existing components, interaction patterns, CSS variables, semantic colour roles, spacing, typography, `docs/ui-pattern-catalogue.md`, and documentation before inventing new UI. If a design source, component story, screenshot baseline, or Figma reference exists, inspect it before changing code.
