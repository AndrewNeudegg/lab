...
Stalled dashboard sends time out after 20 seconds by default, return the composer controls to an interactive state, and keep the failed bubble available for an explicit retry. Set `VITE_HOMELABD_CHAT_SEND_TIMEOUT_MS` at dashboard build or dev-server start time to tune that window. Operators should check the task queue before retrying a timed-out task-creation command.
