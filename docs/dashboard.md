...
- `/assistant`: proactive Assistant runs, recommendation decisions, returned receipts, evidence snapshots, and a secondary capability reference.
...
- Assistant proactive output lives at `/assistant`. The primary page workflow is `GET /assistant/runs`, `GET /assistant/runs/<run_id>`, and `POST /assistant/runs`; run ids are durable records in the control-plane data directory. The secondary reference panel calls `GET /assistant` with optional `area` and `q` query parameters so the control-plane API still owns capability names, related surfaces, autonomy labels, safeguards, and workflow templates.
...
1. What activity makes the operator's life easier?
2. Which Assistant capability supports it?
3. What autonomy level and safeguards apply?
4. Which UI pattern makes the behaviour understandable and controllable?
5. Which workflow template or existing dashboard surface can operationalise it?
6. What did the proactive Assistant notice, what evidence did it use, and what action does it recommend?
