...
The supervisor owns the next-step decision. Workers should not independently decide the long-term roadmap. After every accepted linked task, the supervisor reads the latest structured report, records claims and evidence on the selected milestone, and creates a read-only challenge before accepting that milestone. Failed challenges become gap records; open gaps take priority over new feature work and create `gap_fix` tasks. A build or gap-fix task can claim completion, but only a passing challenge can accept the milestone. When the planned milestones are exhausted and all gaps are closed, the supervisor appends a `Final whole-goal audit` milestone instead of marking the Goal done. Completion requires that final audit to pass with `goal_complete:true`; a scoped milestone challenge with `goal_complete:false` schedules the audit, and an uncertified final audit blocks or creates repair gaps. Remote build Goals must also prove a clean, committed, or intentionally captured target repository state before final completion. The supervisor only stops for a real human, external, policy, safety, budget, repeated no-progress, or impossible-objective blocker. If a task reports an actionable open gap and no operator question, Autopilot records that as agent-owned repair work, consumes the blocking task, and creates the next `gap_fix` task instead of asking the operator to interpret it. A budget of `-1` means unlimited tasks, but unlimited does not override human questions, policy, review gates, final audit, or stop commands.
...
`blockers` should mean work cannot continue autonomously: missing credentials, unsafe or destructive action, external outage, or a product decision. Ordinary open repair gaps belong in `follow_ups` and `gap_ids`. If a blocker is unavoidable, include `blocker_resolver` as `human`, `agent`, or `external`, and `next_action` as the shortest actionable instruction. `agent` means the harness should continue by creating repair work; `human` means the operator must decide or provide something. A reviewed final whole-Goal audit with `goal_complete:true` and no gaps is authoritative: it closes stale open or in-progress gap records before the supervisor completes the Goal, so old blocker traces cannot keep Autopilot blocked after successful certification.
...
Only the explicit `phase_final_audit_milestone_01` challenge may complete a Goal. It must inspect every success criterion, constraints, current repo state, delivery state, and recent challenge history. For remote build Goals, a dirty worktree or undelivered diff is a high delivery gap unless the Goal explicitly accepts that state.
