...
On compact screens `/chat` keeps chat-session history above the selected conversation as a short two-column switcher. The history header owns the `New chat` and clear-all icon buttons; the selected-chat toolbar owns only the current-context clear icon. Keep at least several recent chats visible at once, and keep the transcript and compact composer as the primary working area.
