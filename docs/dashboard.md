...
On compact screens `/chat` keeps chat-session history above the selected conversation as a short two-column switcher. The history header owns `New chat` and `Clear all`; the selected-chat toolbar owns only the current-context `Clear` action. Keep at least several recent chats visible at once, and keep the transcript and composer as the primary working area.
