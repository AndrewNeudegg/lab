...
- When `discover_sources` is enabled, the run sends the model-planned search queries to the registered `internet.research` tool with fetched web pages, imports usable candidates as normal URL sources, runs model source analysis, and then synthesises over the expanded corpus. Search, fetch, model, and import failures stay visible on the run; the executor does not substitute fabricated source content.
