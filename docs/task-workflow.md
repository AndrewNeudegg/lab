...
- `running`: a local in-memory worker or remote agent owns the task. Local stale workers can be recovered by the task supervisor; running remote tasks stay remote-owned and are not taken over by a local worker just because `homelabd` has no in-memory worker handle. Next transition: `running -> ready_for_review` when it finishes, `running -> timed_out` when the external worker reaches its configured deadline, or `running -> blocked` when another failure stops work.
- `ready_for_review`: work is staged in a task worktree, or a legacy remote agent result has been recorded for evidence-only review. Local tasks and Git-backed remote tasks enter the merge queue, and only the queue head can run review. Merge-backed next transition: `ready_for_review -> awaiting_approval` when checks and premerge pass, `ready_for_review -> conflict_resolution` when the task branch cannot reconcile with current `main`, or `ready_for_review -> blocked` for other failures. Legacy remote next transition: `ready_for_review -> awaiting_verification` when review verifies the captured remote evidence, `ready_for_review -> no_change_required` for an explicit no-change result with no diff, or `ready_for_review -> blocked` when the diff, validation, or Goal alignment is insufficient.
...
- `timed_out`: an external local worker or remote agent reached `external_agents.<backend>.timeout_seconds`. This is distinct from `blocked` because the harness owns the first recovery attempt: it retries with timeout-specific instructions, preserves the same remote target for remote tasks, and asks for a smaller coherent slice. Next transition: `timed_out -> queued` or `running` through automatic recovery, `retry`, `delegate`, `run`, or `reopen`. After bounded recovery attempts are exhausted it becomes a manual blocker.
...
- `awaiting_verification`: the reviewed merge has landed in the local repo or the advertised remote checkout, or a legacy remote review acknowledged the remote result. The human still needs to verify the running app or the named remote machine/directory. Next transition: `awaiting_verification -> done` via `accept`, or `awaiting_verification -> queued` via `reopen`.
...
Goal-linked tasks include `goal_id`, `goal_kind`, `execution_mode`, and the Goal target. `goal_kind` tells the UI and worker whether the work came from a Build, Routine, Watch, or Maintenance Goal. `execution_mode` distinguishes Guided tasks, where the operator remains responsible for approval and acceptance, from Autopilot tasks, where the Goal supervisor can create the next bounded task and pass applicable review, merge, restart, and acceptance gates when checks succeed. A remote Autopilot task is queued for the selected `homelab-agent`; it is not started by the local worker supervisor. Git-backed remote Autopilot tasks use the same merge queue as local tasks after the remote agent completes, while legacy remotes use evidence-only review. If a linked Autopilot task times out, the supervisor keeps the Goal running and queues automatic timeout recovery before exposing a human blocker. If a remote agent restarts after claiming a Git-backed task, the coordinator treats the orphaned running task as `timed_out` and requeues the same remote recovery path. If the current linked task was cancelled, an explicit Autopilot resume discards that attempt and selects the next task on the same Goal target; operators should pause or stop the Goal when they intend to halt the objective itself. Assistant-created follow-up tasks from task or Goal signals carry the source Goal id and remote target, so a dashboard surface such as `/tasks` or `/assistant` does not accidentally reroute remote Goal work into the control-plane repo. The worker prompt receives the Goal objective, success criteria, constraints, autonomy, target context, and an explicit report-back contract. When the operator or Assistant Autopilot accepts the task, `homelabd` records a Goal progress note, updates linked task state, and extracts watch recommendations from concise result markers such as `goal.watch.recommend:` or `watch_recommendation:`. The stored Goal task report includes the worker claim plus a reviewer decision, so a diff is not automatically considered progress: repeated ordinary build reports with `no_change`, `insufficient_evidence`, or `misaligned` stop Autopilot, while challenge and gap-fix no-change reports are treated as review evidence rather than wasted build attempts. Before selecting the next task, the supervisor reconciles milestone state from structured task reports: later passing challenges accept a milestone, closed challenge gaps make a milestone eligible for re-challenge, and stale `challenged` states are not allowed to block the plan when no open gap remains. Diff-backed reports with validation and Goal alignment continue the plan. Once the plan is exhausted, Autopilot creates a final whole-goal audit task; only that audit can complete the Goal, and remote build audits must treat dirty or undelivered worktrees as delivery gaps. Use Goal-linked tasks when a durable objective needs concrete work while preserving the larger desire and its future checks.
...
The dashboard `/tasks` selected-task record renders the same data in the `Changes vs main` panel. Reviewed local git tasks and Git-backed remote tasks store an immutable review snapshot before merge approval so the historical patch remains available after merge and accept. Legacy remote tasks store an immutable task-scoped snapshot from the remote agent when available, including untracked files but excluding agent telemetry paths. Older records can fall back to a live branch or remote worktree diff; those responses are labelled with a warning because later work may have changed the checkout. The panel provides provenance, changed-file navigation, split and unified views with line numbers, wrapped long lines, and inline changed-text highlights.
...
When a remote agent finishes a Git-backed task, `homelabd` records the result, stores the captured remote diff, and moves the task to `ready_for_review`. The normal review gate then commits the task worktree, compares it with the advertised checkout `HEAD`, runs configured checks from that worktree, stores an immutable review snapshot, and creates a merge approval. The approved merge lands in the advertised remote checkout, not the control-plane checkout. Read-only Goal challenge tasks are the exception to the diff requirement: if a Git-backed remote challenge produces no diff, review parses the `GOAL_CHALLENGE` report and lets the Goal supervisor pass the milestone, create gap-fix work, or reject malformed challenge output. The challenge prompt includes the exact JSON Schema; `claims_challenged` is an array of strings, and each `gaps` item includes `area`, `claim`, `severity`, `evidence`, and `suggested_task`. Schema failures are reported with the exact mismatched path, then recovered automatically where possible by deterministic normalisation or LLM reformatting against the same schema. If recovery cannot produce valid JSON, the task supervisor retries the remote task with a format-only instruction before the blocker is escalated to the operator. It must not ask the operator to decide whether no diff is acceptable for a challenge task. If the advertised checkout is dirty before task creation or claim, the task is blocked with a dirty-checkout reason instead of hiding that state in the next baseline. Legacy remote tasks for non-Git or inaccessible directories keep the evidence-only review path: review inspects the captured evidence and moves to `awaiting_verification`, `no_change_required`, or `blocked` without a merge approval.
