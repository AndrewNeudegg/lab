...
`POST /claim` offers the next queued task, but the task remains `queued` until the agent calls `POST /tasks/<task_id>/ack` with the supplied `attempt_id`. Only that acknowledgement marks the task `running`, sets the remote agent's current task and attempt, and starts the execution deadline. If the claim response times out, the next claim returns the same offered attempt until it is acknowledged or the short offer lease expires. This keeps the coordinator from showing a task as running when the remote process never received the assignment.

The agent executes in the selected task directory after acknowledgement, sends stdout/stderr back as the task result, and captures the remote git working-tree patch after completion. The captured patch includes uncommitted tracked changes and untracked files, excluding ignored runtime state such as `.agent-*`. Before calling back to `homelabd`, the agent writes the completion payload, including the `attempt_id`, to `data/remote-agent/pending-completions/<agent_id>/` under its configured `data_dir`. If `homelabd` is restarting, offline, or returns a transient network error, the remote agent keeps advertising that task and attempt as current and replays the saved completion before claiming new work. A successful callback deletes the pending completion file. While a remote task is `running`, the control plane must not recover it with a local worker simply because there is no in-memory local worker handle; the remote agent's heartbeat, acknowledged attempt, timeout, and pending-completion replay own that lifecycle. The same backend `timeout_seconds` applies to remote task execution; omitted or zero values default to 3,600 seconds, or 1 hour. `homelabd` adds a short callback grace period before declaring the acknowledged attempt timed out. If the worker or coordinator deadline is reached, the remote task is recorded as `timed_out` rather than `blocked`. The task supervisor treats timed-out tasks, interrupted remote worktree preparation, and remote diff capture failures as bounded automatic recovery candidates, requeues the same remote target with failure-specific instructions, clears any stale Goal-level blocker for the current Autopilot task, and only exposes a manual blocker after recovery attempts are exhausted. Dirty advertised checkouts are still operator-owned blockers because the coordinator must not hide uncommitted work. The default Codex backend disables Codex's own sandbox because tasks already run in isolated worktrees and Codex otherwise may remount `.git` read-only, which prevents Git worktree metadata updates.
...
- `POST /agents/{id}/claim` offers the next queued task targeted to that agent and returns an `attempt_id`.
- `POST /agents/{id}/tasks/{task_id}/ack` acknowledges the offered `attempt_id` and moves that task to `running`.
- `POST /agents/{id}/tasks/{task_id}/complete` records completion for the acknowledged `attempt_id`. Remote agents should report `completed`, `no_change_required`, `failed`, `blocked`, or `timed_out`. Duplicate completion posts for an already-recorded remote task are idempotent so an agent can delete a replayed pending completion after a coordinator restart. Stale completions for older attempts are ignored rather than allowed to overwrite the current attempt.
...
3. The remote agent receives an offered attempt, acknowledges it, then runs the worker in that task worktree.
4. The remote agent reports output, validation, attempt id, and the task-scoped diff back to `homelabd`.
...
Use `homelabctl task diff <task_id>` or the dashboard `Changes vs main` panel to inspect the task diff snapshot. Reviewed Git-backed remote tasks store the same immutable branch-vs-target snapshot as local tasks, so the historical patch remains visible after merge. Legacy remote completions store the task-scoped snapshot reported by the agent when available. The structured diff response includes `source`, `snapshot`, `captured_at`, and `sha256` provenance. Reopening or retrying a remote task archives the previous remote attempt and clears the current diff fields so the next attempt cannot accidentally display a stale merged patch as live work. If a completion predates task-scoped snapshots and the workdir path is still accessible from the control plane, `GET /tasks/{task_id}/diff` may compute a live remote working-tree fallback and marks it with a warning. If neither source is available, the task still shows the remote result and validation, but the diff panel tells the operator that no remote diff is available.
