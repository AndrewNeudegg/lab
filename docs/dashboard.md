...
- Automatic sync refreshes tasks, approvals, events, remote agents, and remote project workspaces on the page interval, then refreshes selected-task worker runs and the local diff without blocking the queue from becoming current. Do not expose a manual refresh button on `/tasks`; the header status should show connectivity and retry state instead.
