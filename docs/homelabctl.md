...
`task retry` preserves the previous task result as retry context and forces an immediate worker attempt. Use it when the operator wants to override the automatic recovery path, for example after raising `external_agents.<backend>.timeout_seconds` or adding narrower instructions. The task supervisor queues bounded automatic recovery for `timed_out`, `conflict_resolution`, and retryable blocked tasks; remote recovery preserves the same remote agent and workdir. Worker starts are capped by `limits.max_concurrent_tasks`, so local recovery waits instead of launching too many browser-UAT or build-heavy tasks at once. In both paths, `homelabd` prepares the isolated task worktree before starting the worker: a clean worktree is merged with current `main`, and any resulting conflicts are left for the worker to resolve.
...
`task review` normally runs after a worker has moved the task to `ready_for_review`; the task supervisor starts that review automatically for local tasks and for Git-backed remote tasks whose agent has `auto-review` enabled. It can also recheck a blocked task whose result starts with `ReviewerAgent checks failed:` after a test-infrastructure fix, and it can recheck a legacy remote task blocked by `ReviewerAgent could not verify remote Goal progress:` after recovered evidence or parser fixes. Running Autopilot Goals perform that remote Goal recheck themselves before asking for operator input, so stale validation blockers should not become manual decisions. It owns the task while checks run; concurrent run, retry, or delegation attempts are rejected, and a stale review result is ignored if the task state changes before checks finish. If the worker made no edits and its result starts with `No change required:`, review moves the task to `no_change_required`; use `task accept` to close it or `task reopen <reason>` to reject that conclusion and queue new work.
...
`settings auto-merge <on|off>` toggles whether `homelabd` automatically grants merge approvals for the local merge-queue head after review passes. Auto merge still respects the queue order, pre-merge reconciliation, required restart gates, and health checks. Leave it off when you want every local merge approval to require an operator click.

Remote agents have their own policy. Use `settings remote-agent <agent_id> auto-review <on|off>` to let the task supervisor review that agent's completed tasks, `settings remote-agent <agent_id> auto-merge <on|off>` to grant the resulting merge approval for Git-backed remote tasks, or `settings remote-agent <agent_id> auto-review-merge <on|off>` to change both at once. Git-backed remote auto-merge lands the task branch in the advertised remote checkout and then waits in `awaiting_verification`, matching local tasks. Legacy evidence-only remotes still use the older meaning: after independent review, auto-merge accepts the reviewed result because no merge target exists.
...
`task diff` and its top-level `diff` alias call `GET /tasks/{task_id}/diff`. Plain output starts with a compact file/addition/deletion summary, prints the diff source and any warning, then prints the raw patch. Reviewed local tasks and Git-backed remote tasks return immutable review snapshots. Legacy remote tasks return the agent's task snapshot when available; otherwise the response is marked as a live remote worktree fallback so operators know it may not match historical task state. Add `-json` to inspect the structured response used by the dashboard diff viewer, including `source`, `snapshot`, `captured_at`, and `sha256`.
...
go run ./cmd/homelabctl goal answer --question "Support all platforms?" goal_123 "Record product-owner waivers before claiming unsupported platforms."
...
`GET /assistant/runs?limit=<n>` returns bounded active run summaries plus `counts` for active, archived, and total runs. `GET /assistant/runs?archived=only&limit=<n>` returns a bounded archived page with the same counts. Use `detail=full` only when a caller really needs full run payloads; the dashboard normally loads a single selected run through `GET /assistant/runs/<run_id>`.

`assistant signals` calls `GET /assistant/signals` for the current unresolved signal inbox. `assistant signal` calls `PATCH /assistant/signals/<fingerprint>` with `useful`, `dismiss`, `snooze`, or `create_task`, so operators and agents can feed the Assistant learning loop or create a bounded follow-up task without ad hoc HTTP calls. `create_task` records the created task on the signal and settles matching active run recommendations with the same fingerprint so duplicate recommendations leave the active queue. Assistant-owned surfaces such as `assistant`, `chat`, `tasks`, `dashboard`, `healthd`, and `supervisord` create local control-plane tasks unless the action carries an explicit remote target or a linked Goal/source task with its own remote target. `useful` records positive feedback and clears the current inbox item until a later new sighting reopens it.

Goals are durable operator desires rather than one-off tasks. A Goal stores the objective, type, execution mode, target, details, success criteria, constraints, autonomy, cadence, watches, linked tasks, progress notes, run assessments, and, for Autopilot, a supervisor plan. Goal types are `build`, `routine`, `watch`, and `maintenance`. `guided` mode keeps the human in the loop: `goal check` starts a Goal-scoped Assistant run through `POST /assistant/goals/<goal_id>/check` and the operator decides whether to create or accept work. `autopilot` mode lets the Goal create one bounded linked task at a time on its selected local or remote target, use the normal review, merge, restart, and acceptance gates where applicable, and continue until its task limit, runtime limit, human/external blocker, no-progress stop, completion, or stop command is reached. `--budget-tasks` is the API field for the Autopilot task limit; pass a positive integer for a bounded Goal or `-1` for unlimited work. Unlimited work still stops on open questions, policy limits, failed gates that are not agent-repairable, final audit, or explicit stop commands. `goal show <goal_id> -json` includes `goal.plan`, `blocker_trace`, `decisions`, and `task_reports`, so operators and agents can see the selected phase, milestone, current blocker, latest supervisor decision, structured worker report, challenge verdicts, open gaps, and independent reviewer decision for each accepted linked task. `blocker_trace.resolver` tells clients who owns the next action: `human`, `agent`, or `external`; `next_action` is the plain-language action. `blocker_trace.flow` is the canonical API action model for clients: it names the role, title, valid choices, and whether a Goal question can be answered. Clients must not infer answerability from `blocker_trace.questions`; task-report questions are evidence unless `blocker_trace.source_type` is `open_questions` and the flow role is `goal_question`. When a blocked Autopilot Goal has a current linked task in its stop reason, that current task is the active blocker and older task-report evidence must not be exposed as the decision target. Repairable current-task blockers expose API-provided choices such as retrying the task, retrying with instruction, or reopening with new direction, so clients do not have to invent the unblock path. When `blocker_trace.source_type` is `open_questions`, the blocking object is the Goal question, not the closed source task. Use `goal answer [--question TEXT] <goal_id> <answer>` or the Assistant UI decision panel to record the operator answer as a Goal note and supervisor decision, clear that open question, and resume Autopilot by default. Pass `--resume=false` to only record the answer. The answer endpoint returns a conflict rather than a server error when a client tries to answer a question that is not currently open on the Goal. The answer is copied into Goal progress context so later tasks can act on the decision. `GET /assistant/goals/<goal_id>?limit=<n>` returns the latest timeline items plus `counts` totals for UI use; omit `limit` for the complete timeline. The non-JSON `goal show` output summarises accepted milestones and open gaps for a quick terminal check. The `goal.autopilot.tasks_started` field is the total Autopilot work counter; `goal.linked_tasks` is a capped navigation cache of retained task ids. `goal edit` calls `PATCH /assistant/goals/<goal_id>` and can change existing Goal text, details, target, mode, cadence, autonomy, and task limit. Editing an exhausted Autopilot Goal to a task limit that allows more work resumes the Goal and the harness reconciles it against the new objective. Changing the objective, details, type, success criteria, or constraints revises the durable plan before the next task is selected. `goal autopilot start|pause|resume|stop` calls `POST /assistant/goals/<goal_id>/autopilot/<action>`. Resuming an Autopilot Goal with a cancelled current task discards that attempt and selects the next task on the same Goal target; cancelling a task is not the same as stopping the Goal. `goal watch` records something the Assistant should keep an eye on. Accepted Goal-linked build and gap-fix tasks report progress back to the Goal with a `GOAL_REPORT:` JSON line containing `task_type`, `phase_id`, `milestone_id`, summary, advancement, changed files, validation, claims, follow-ups, blockers, questions, optional `gap_ids`, optional `blocker_resolver`, and optional `next_action`. The task prompt includes the exact JSON Schema for that line; `claims` must be objects with `claim` and string-array `evidence`. The harness treats that line as a claim: it separately reviews diff files, validation, and Goal alignment before creating a read-only challenge task. `blockers` are reserved for conditions that genuinely stop autonomous work; open repair gaps should be `follow_ups` plus `gap_ids` so Autopilot can continue. Challenge tasks report with `GOAL_CHALLENGE:` containing milestone ID, verdict, evidence, challenged claims, gaps, and `goal_complete`; the challenge prompt includes the exact JSON Schema, including `claims_challenged` as an array of strings and `gaps` as objects with `area`, `claim`, `severity`, `evidence`, and `suggested_task`. Passing milestone challenges accept milestones but do not complete the Goal. Before choosing a task, the supervisor reconciles stale plan status from task reports: a later passing challenge accepts its milestone, a challenged milestone with closed gaps is re-challenged, and stale `challenged` states cannot produce a human blocker by themselves. When all planned milestones are accepted and gaps are closed, the supervisor schedules `phase_final_audit_milestone_01` as a final whole-goal audit. Only that audit can complete the Goal, and only when it passes with `goal_complete:true`; remote build Goals must show a clean, committed, or intentionally captured target repo state. A passing final audit with no gaps closes stale open or in-progress gap records before completion, because the audit supersedes old blocker snapshots. Failed or uncertified audits create gaps or block with a clear reason. Failed ordinary challenges create open gaps, and the supervisor creates `gap_fix` tasks before broadening the plan. The repeated no-progress stop applies to ordinary build reports; challenge and gap-fix no-change reports are review evidence and do not count as wasted build attempts. If a Git-backed remote agent restarts after claiming a task, the coordinator marks the orphaned running task `timed_out` and queues automatic remote recovery on the same agent/workdir. If structured Goal output fails schema validation, review records the exact mismatched path such as `GOAL_CHALLENGE.claims_challenged[0] must be string`, first tries automatic normalisation or LLM reformatting against the schema, then retries the task with format-only recovery before exposing it as a human blocker. If a blocked task produced actionable open gaps and no operator question, the supervisor consumes that task as agent-owned repair evidence and creates the next `gap_fix` task instead of exposing it as a human blocker. Build Goals also tell workers to maintain a feature or parity matrix in the target repo so each new task can close a concrete slice instead of broadly “continuing”. Chat accepts the same lifecycle through natural phrases such as `goal keep invoices reconciled` or `my goal is to keep the daily brief ready`.
...
- `POST /assistant/goals/{id}/questions/answer`
